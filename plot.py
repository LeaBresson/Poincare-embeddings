...
# Taken from: https://github.com/TatsuyaShirakawa/poincare-embedding
...
def plot_embeddings2d(embeddings, edges_df, center_mammal, center_node='mammal.n.01'):
...
def plotly_embeddings2d(embeddings, edges_df, center_mammal=True, name='blank', center_node='mammal.n.01'):
    def apply_center(row):
        x, y = isom((row[0], row[1]))
        return [x, y]
...
def plotly_embeddings3d(embeddings, edges_df):
    # Create a trace
    nodes = go.Scatter3d(
        x=embeddings.x1,
        y=embeddings.x2,
